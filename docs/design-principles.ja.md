...
Web API は、UseCase を外部に公開するための入口にすぎない。
...
そのため、UseCase は HTTP を知らない。  
UseCase は ASP.NET Core を参照しない。  
UseCase は Controller や Minimal API に依存しない。

---

### 2.3 C# と F# を同じ水系に流す

Kawa は、C# と F# の両方でドメインモデルやユースケースを書けることを重視する。
...
contract-first な RPC 層として、MagicOnion や gRPC との統合を検討する。
